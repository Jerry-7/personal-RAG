...
| **研究 Agent** | 在自动、本地、网页三种模式间切换，统一引用本地文档、笔记与网页快照 |
| **研究笔记** | 从对话创建可编辑草稿，发布后可手动加入 RAG，并用标签和集合组织 |
...
网页模式使用本机 SearXNG。配置已显式开放 JSON Search API，并仅监听回环地址：
...
默认地址为 `http://127.0.0.1:8888`，可在应用“设置 → 网页研究”中修改。部署前请替换 `deploy/searxng/settings.yml` 中的本地密钥。网页读取仅支持公开 HTTP(S) HTML/PDF，并限制同域深度、页面数、响应大小和请求频率。
...
3. 等待文件状态变为 **"已就绪"**
4. 选择“自动 / 本地 / 网页”模式后提问
5. 点击回答中的 `[1]` `[2]` 标记查看固定版本的来源
6. 点击“整理成笔记”，编辑后发布；需要检索该笔记时再点击“加入知识库”
