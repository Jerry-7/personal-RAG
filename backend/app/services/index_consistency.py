...
from app.db.models import Chunk, Document, IndexJob
...
def repair_vector_index(db: Session) -> dict[str, int]:
    ...
    if missing_ids:
        affected_documents = {
            row[0]
            for row in db.query(Chunk.document_id).filter(Chunk.id.in_(missing_ids)).distinct().all()
        }
    ...
    for doc_id in affected_documents:
        ...
        if job:
            ...
            db.add(IndexJob(document_id=doc_id, status="queued", config_json="{}"))
